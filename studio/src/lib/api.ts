...
let detectedHealthyBase: string | null = null

async function isHealthyApiBase(base: string): Promise<boolean> {
...
async function prioritizeHealthyBase(bases: string[]): Promise<string[]> {
  if (bases.length <= 1) return bases
  if (detectedHealthyBase && bases.includes(detectedHealthyBase)) return [detectedHealthyBase, ...bases.filter((base) => base !== detectedHealthyBase)]

  for (const base of bases) {
    if (await isHealthyApiBase(base)) {
      detectedHealthyBase = base
      return [base, ...bases.filter((candidate) => candidate !== base)]
    }
  }
...
export async function apiFetch(path: string, init?: RequestInit): Promise<Response> {
  const normalizedPath = normalizePath(path)
  const bases = await prioritizeHealthyBase(resolveApiBases())
